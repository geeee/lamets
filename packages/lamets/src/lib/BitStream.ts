...
import { new_byte, new_int, fill, arraycopy, assert } from './common.js';
import { NORM_TYPE, SHORT_TYPE } from './constants.js';
...
/**
 * Maximum MP3 buffer size. Matches Lame.LAME_MAXMP3BUFFER.
 */
...
function CRC_update(value: number, crc: number): number {
  value <<= 8;
  for (let i = 0; i < 8; i++) {
    value <<= 1;
    crc <<= 1;
...
export class BitStream {
  ...
  init(ga: GainAnalysis, ver: Version, vbr: VBRTag): void {
    this.ga = ga;
    this.ver = ver;
    this.vbr = vbr;
  }
  ...
  getframebits(gfp: LameGlobalFlags): number {
    ...
    const bytes = ((gfp.getMpegVersion() + 1) * 72000 * bit_rate / gfp.getOutSampleRate() + gfc.padding) | 0;
    return 8 * bytes;
  }

  private putheader_bits(gfc: LameInternalFlags): void {
    arraycopy(gfc.header[gfc.w_ptr].buf, 0, this.buf, this.bufByteIdx,
      gfc.sideinfo_len);
   ...
   */
  putbits2(gfc: LameInternalFlags, val: number, j: number): void {
    ...
    while (j > 0) {
      ...
      assert(this.bufBitIdx < MAX_LENGTH);
   ...
   */
  private putbits_noheaders(gfc: LameInternalFlags, val: number, j: number): void {
    assert(j < MAX_LENGTH - 2);
   ...
   * the ancillary data...
   */
  drain_into_ancillary(gfp: LameGlobalFlags, remainingBits: number): void {
    const gfc = gfp.internal_flags!;
    let i: number;
    ...
    if (remainingBits >= 32) {
      const versionStr = this.ver.getLameShortVersion();
      if (remainingBits >= 32) {
        for (i = 0; i < versionStr.length && remainingBits >= 8; ++i) {
          remainingBits -= 8;
          this.putbits2(gfc, versionStr.charCodeAt(i), 8);
        }
      }
    }
  ...
  CRC_writeheader(gfc: LameInternalFlags, header: Uint8Array): void {
    let crc = 0xffff;
  ...
  private encodeSideInfo2(gfp: LameGlobalFlags, bitsPerFrame: number): void {
    const gfc = gfp.internal_flags!;
    let l3_side: IIISideInfo;
    let gr: number, ch: number;

    l3_side = gfc.l3_side;
    gfc.header[gfc.h_ptr].ptr = 0;
    fill(gfc.header[gfc.h_ptr].buf, 0, 0, gfc.sideinfo_len);
    ...
    this.writeheader(gfc, gfp.getMode() as number, 2);
    ...
    if (gfp.error_protection) {
      this.writeheader(gfc, 0, 16); /* dummy */
    }
    ...
    if (gfp.getMpegVersion() === 1) {
      ...
      for (gr = 0; gr < 2; gr++) {
        for (ch = 0; ch < gfc.channels_out; ch++) {
          ...
          this.writeheader(gfc, (gi.big_values / 2) | 0, 9);
          this.writeheader(gfc, gi.global_gain, 8);
          this.writeheader(gfc, gi.scalefac_compress, 4);

          if (gi.block_type !== NORM_TYPE) {
            this.writeheader(gfc, 1, 1); /* window_switching_flag */
            ...
            this.writeheader(gfc, 0, 1); /* window_switching_flag */
            if (gi.table_select[0] === 14) {
              gi.table_select[0] = 16;
            }
            this.writeheader(gfc, gi.table_select[0], 5);
  ...
  private huffman_coder_count1(gfc: LameInternalFlags, gi: GrInfo): number {
    /* Write count1 area */
    const h = this.tables.ht[gi.count1table_select + 32];
    let i: number, bits = 0;
    ...
    for (i = ((gi.count1 - gi.big_values) / 4) | 0; i > 0; --i) {
      let huffbits = 0;
      let p = 0;
      let v: number;
  ...
  private Huffmancode(
    ...
    const h = this.tables.ht[tableindex];
    let bits = 0;
  ...
  private LongHuffmancodebits(gfc: LameInternalFlags, gi: GrInfo): number {
    let bigvalues: number, bits: number;
    let region1Start: number, region2Start: number;
  ...
  private writeMainData(gfp: LameGlobalFlags): number {
    let gr: number, ch: number, sfb: number, data_bits: number, tot_bits = 0;
    const gfc = gfp.internal_flags!;
    ...
    if (gfp.getMpegVersion() === 1) {
      ...
      for (ch = 0; ch < gfc.channels_out; ch++) {
        const gi = l3_side.tt[gr][ch];
        let i: number, sfb_partition: number, scale_bits = 0;
        ...
        if (gi.block_type === SHORT_TYPE) {
          for (; sfb_partition < 4; sfb_partition++) {
            const sfbs = (gi.sfb_partition_table![sfb_partition] / 3) | 0;
  ...
  private compute_flushbits(
    ...
    const gfc = gfp.internal_flags!;
    let flushbits: number, remaining_headers: number;
    let bitsPerFrame: number;
    let last_ptr: number, first_ptr: number;
    first_ptr = gfc.w_ptr;
    ...
    if ((total_bytes_output.total % 8) !== 0) {
      total_bytes_output.total = 1 + ((total_bytes_output.total / 8) | 0);
    } else {
      total_bytes_output.total = (total_bytes_output.total / 8) | 0;
    }
    total_bytes_output.total += this.bufByteIdx + 1;

    if (flushbits < 0) {
      console.error("strange error flushing buffer ... \n");
    }
    return flushbits;
  }
  ...
  flush_bitstream(gfp: LameGlobalFlags): void {
    ...
    if (gfc.findPeakSample) {
      gfc.noclipGainChange = (Math.ceil(
        Math.log10(gfc.PeakSample / 32767.0) * 20.0 * 10.0)) | 0;
      /* round up */
  ...
  add_dummy_byte(gfp: LameGlobalFlags, val: number, n: number): void {
    const gfc = gfp.internal_flags!;
    let i: number;
  ...
  format_bitstream(gfp: LameGlobalFlags): number {
    ...
    l3_side.main_data_begin += ((bitsPerFrame - bits) / 8) | 0;
    ...
    if ((l3_side.main_data_begin * 8) !== gfc.ResvSize) {
      ...
      console.error("This is a fatal error.  It has several possible causes:");
      console.error("90%  LAME compiled with buggy version of gcc using advanced optimizations");
      console.error(" 9%  Your system is overclocked");
      console.error(" 1%  bug in LAME encoding library");
  ...
  copy_buffer(
    ...
    arraycopy(this.buf, 0, buffer, bufferPos, minimum);
    this.bufByteIdx = -1;
    this.bufBitIdx = 0;
    ...
    if (mp3data !== 0) {
      ...
      if (gfc.decode_on_the_fly) {
        ...
        while (samples_out !== 0) {
          ...
          samples_out = 0;
          // mpg.hip_decode1_unclipped not available in TS port
          // In lamejs this calls mpg.hip_decode1_unclipped(gfc.hip, buffer,
          //   bufferPos, mp3_in, pcm_buf[0], pcm_buf[1]);
          // Since we don't have the MPGLib decoder, skip decode_on_the_fly.
          // The original Java code does this:
          //   samples_out = mpg.hip_decode1_unclipped(gfc.hip, buffer,
          //     bufferPos, mp3_in, pcm_buf[0], pcm_buf[1]);

          /*
           * set the length of the mp3 input buffer to zero, so that
          ...
          if (samples_out > 0) {
            ...
            if (gfc.findReplayGain) {
              if (this.ga.AnalyzeSamples(gfc.rgdata!, pcm_buf[0], 0,
  ...
  init_bit_stream_w(gfc: LameInternalFlags): void {
    this.buf = new_byte(LAME_MAXMP3BUFFER);
