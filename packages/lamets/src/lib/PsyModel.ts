import { new_float, new_float_n, new_int, fill, assert } from './common.js';
import {
  BLKSIZE,
  HBLKSIZE,
  BLKSIZE_s,
...
function NON_LINEAR_SCALE_ENERGY(x: number): number {
  return x;
}
...
export class PsyModel {
  ...
  private static readonly rpelev = 2;
  private static readonly rpelev2 = 16;
  ...
  private static readonly VO_SCALE = 1.0 / (14752 * 14752) / (BLKSIZE / 2);
   ...
   * pow(10, -0.0..-0.6)
   */
  private static readonly tab: number[] = [
  ...
  private static readonly table3: number[] = [
    2.35364 * 2.35364,
  ...
  private static readonly table2_: number[] = [
    1.33352 * 1.33352,
    1.35879 * 1.35879, 1.38454 * 1.38454, 1.39497 * 1.39497,
    1.40548 * 1.40548, 1.3537 * 1.3537, 1.30382 * 1.30382,
    1.22321 * 1.22321, 1.14758 * 1.14758, 1,
  ...
  private static readonly regcoef_s: number[] = [
    11.8, 13.6, 17.2, 32, 46.5,
    51.3, 57.5, 67.1, 71.5, 84.6, 97.6, 130,
    /* 255.8 */
  ];

  /** these values are tuned only for 44.1kHz... */
  private static readonly regcoef_l: number[] = [
    6.8, 5.8, 5.8, 6.4, 6.5, 9.9,
    12.1, 14.4, 15, 18.9, 21.6, 26.9, 34.2, 40.2, 46.8, 56.5,
    60.7, 73.9, 85.7, 93.4, 126.1,
    /* 241.3 */
  ];

  private static readonly fircoef: number[] = [
    -8.65163e-18 * 2, -0.00851586 * 2, -6.74764e-18 * 2, 0.0209036 * 2,
    -3.36639e-17 * 2, -0.0438162 * 2, -1.54175e-17 * 2, 0.0931738 * 2,
    -5.52212e-17 * 2, -0.313819 * 2,
  ];

  private static readonly fircoef_: number[] = [
    -8.65163e-18 * 2, -0.00851586 * 2, -6.74764e-18 * 2, 0.0209036 * 2,
    -3.36639e-17 * 2, -0.0438162 * 2, -1.54175e-17 * 2, 0.0931738 * 2,
    -5.52212e-17 * 2, -0.313819 * 2,
  ];
   ...
   *      gfc      - uses out_samplerate, ATHtype
   * returns: loudness^2 approximation
   */
  ...
  private compute_ffts(
    ...
    for (let j = BLKSIZE / 2 - 1; j >= 0; --j) {
      ...
      fftenergy[BLKSIZE / 2 - j] = NON_LINEAR_SCALE_ENERGY(
        (re * re + im * im) * 0.5,
      );
    }
    for (let b = 2; b >= 0; --b) {
      ...
      for (let j = BLKSIZE_s / 2 - 1; j >= 0; --j) {
        ...
        fftenergy_s[b][BLKSIZE_s / 2 - j] = NON_LINEAR_SCALE_ENERGY(
          (re * re + im * im) * 0.5,
        );
      }
    }
  ...
  private mask_add(
    ...
    if ((b + 3) <= 3 + 3) {
      ...
      /* 22% of the total */
      let i = (FAST_LOG10_X(ratio, 16.0)) | 0;
   ...
   * addition of simultaneous masking   Naoki Shibata 2000/7
   */
  private vbrpsy_mask_add(m1: number, m2: number, b: number): number {
    let ratio: number;
    ...
    if (-2 <= b && b <= 2) {
      ...
      if (ratio >= this.ma_max_i1) {
        ...
        let i = (FAST_LOG10_X(ratio, 16.0)) | 0;
        return (m1 + m2) * PsyModel.table2_[i];
      }
    }
   ...
   */
  private msfix1(gfc: LameInternalFlags): void {
    for (let sb = 0; sb < SBMAX_l; sb++) {
      ...
      let rmid = Math.max(gfc.thm[2].l[sb], Math.min(gfc.thm[3].l[sb], mld));

      mld = gfc.mld_l[sb] * gfc.en[2].l[sb];
      let rside = Math.max(gfc.thm[3].l[sb], Math.min(gfc.thm[2].l[sb], mld));
      gfc.thm[2].l[sb] = rmid;
  ...
  private ns_msfix(
    ...
    msfix2 *= 2.0;
    for (let sb = 0; sb < SBMAX_l; sb++) {
      let thmLR: number, thmM: number, thmS: number, ath: number;
  ...
  private convert_partition2scalefac_s(
    ...
    let sb: number, b: number;
    let enn = 0.0;
    let thmm = 0.0;
    for (sb = b = 0; sb < SBMAX_s; ++b, ++sb) {
      let bo_s_sb = gfc.bo_s[sb];
      let npart_s = gfc.npart_s;
      let b_lim = bo_s_sb < npart_s ? bo_s_sb : npart_s;
      while (b < b_lim) {
        assert(eb[b] >= 0);
        assert(thr[b] >= 0);
        enn += eb[b];
      ...
      assert(thr[b] >= 0);
      {
        ...
        let w_curr = gfc.PSY!.bo_s_weight[sb];
  ...
  private convert_partition2scalefac_l(
    ...
    for (sb = b = 0; sb < SBMAX_l; ++b, ++sb) {
      let bo_l_sb = gfc.bo_l[sb];
      let npart_l = gfc.npart_l;
      let b_lim = bo_l_sb < npart_l ? bo_l_sb : npart_l;
      while (b < b_lim) {
        assert(eb[b] >= 0);
        assert(thr[b] >= 0);
        enn += eb[b];
      ...
      {
        /* at transition sfb -> sfb+1 */
        let w_curr = gfc.PSY!.bo_l_weight[sb];
        let w_next = 1.0 - w_curr;
        enn = w_curr * eb[b];
        thmm = w_curr * thr[b];
        gfc.en[chn].l[sb] += enn;
  ...
  private compute_masking_s(
    ...
    const gfc = gfp.internal_flags!;
    let j: number, b: number;

    for (b = j = 0; b < gfc.npart_s; ++b) {
      let ebb = 0, m = 0;
      let n = gfc.numlines_s[b];
      for (let i = 0; i < n; ++i, ++j) {
        let el = fftenergy_s[sblock][j];
        ebb += el;
        if (m < el) m = el;
      }
      eb[b] = ebb;
    ...
    for (j = b = 0; b < gfc.npart_s; b++) {
...

      {
        ...
        let x = PsyModel.rpelev_s * gfc.nb_s1[chn][b];
        thr[b] = Math.min(ecb, x);
      }
      if (gfc.blocktype_old[chn & 1] === SHORT_TYPE) {
        /* limit calculated threshold by even older granule */
        let x = PsyModel.rpelev2_s * gfc.nb_s2[chn][b];
        let y = thr[b];
        thr[b] = Math.min(x, y);
      }
  ...
  private NS_INTERP(x: number, y: number, r: number): number {
  ...
  private pecalc_s(mr: III_psy_ratio, masking_lower: number): number {
    ...
    for (let sb = 0; sb < SBMAX_s - 1; sb++) {
      for (let sblock = 0; sblock < 3; sblock++) {
        let thm = mr.thm.s[sb][sblock];
        assert(sb < PsyModel.regcoef_s.length);
        if (thm > 0.0) {
          let x = thm * masking_lower;
          let en = mr.en.s[sb][sblock];
          if (en > x) {
            if (en > x * 1e10) {
              pe_s += PsyModel.regcoef_s[sb] * (10.0 * PsyModel.LOG10);
            } else {
              assert(x > 0);
              pe_s += PsyModel.regcoef_s[sb] * FAST_LOG10(en / x);
            }
          }
        }
      }
  ...
  private pecalc_l(mr: III_psy_ratio, masking_lower: number): number {
    ...
    for (let sb = 0; sb < SBMAX_l - 1; sb++) {
      let thm = mr.thm.l[sb];
      assert(sb < PsyModel.regcoef_l.length);
      if (thm > 0.0) {
        let x = thm * masking_lower;
        let en = mr.en.l[sb];
        if (en > x) {
          if (en > x * 1e10) {
            ...
            assert(x > 0);
            pe_l += PsyModel.regcoef_l[sb] * FAST_LOG10(en / x);
          }
        }
      }
    }
  ...
  private calc_mask_index_l(
    ...
    let last_tab_entry = PsyModel.tab.length - 1;
    let b = 0;
    let a = avg[b] + avg[b + 1];
    assert(a >= 0);
    if (a > 0.0) {
    ...
    for (b = 1; b < gfc.npart_l - 1; b++) {
      ...
      if (a > 0.0) {
        ...
        if (m < max[b + 1]) m = max[b + 1];
        assert(
          gfc.numlines_l[b - 1] + gfc.numlines_l[b] + gfc.numlines_l[b + 1] - 1 >
            0,
        );
        a =
          (20.0 * (m * 3.0 - a)) /
  ...
  L3psycho_anal_ns(
    ...
    percep_entropy: Float32Array,
    percep_MS_entropy: Float32Array,
    energy: Float32Array,
    blocktype_d: Int32Array,
  ): number {
    /*
    ...
    let wsamp_L: Float32Array[] = new_float_n(2, BLKSIZE) as Float32Array[];
    let wsamp_S: Float32Array[][] = [
      new_float_n(3, BLKSIZE_s) as Float32Array[],
      new_float_n(3, BLKSIZE_s) as Float32Array[],
    ];

    /* convolution */
    let eb_l = new_float(CBANDS + 1);
    let eb_s = new_float(CBANDS + 1);
    let thr = new_float(CBANDS + 2);

    /* block type */
    let blocktype = new_int(2);
    let uselongblock = new_int(2);

    /* usual variables like loop indices, etc.. */
    let numchn: number, chn: number;
    let b: number, i: number, j: number, k: number;
    let sb: number, sblock: number;

    /* variables used for --nspsytune */
    let ns_hpfsmpl: Float32Array[] = new_float_n(2, 576) as Float32Array[];
    let pcfact: number;

    let mask_idx_l = new_int(CBANDS + 2);
    let mask_idx_s = new_int(CBANDS + 2);

    fill(mask_idx_s, 0);

    numchn = gfc.channels_out;
    /* chn=2 and 3 = Mid and Side channels */
    if (gfp.getMode() === MPEGMode.JOINT_STEREO) numchn = 4;
    ...
    for (chn = 0; chn < gfc.channels_out; chn++) {
      ...
      let firbufPos = bufPos + 576 - 350 - PsyModel.NSFIRLEN + 192;
      assert(PsyModel.fircoef.length === (((PsyModel.NSFIRLEN - 1) / 2) | 0));
      for (i = 0; i < 576; i++) {
        let sum1: number, sum2: number;
        sum1 = firbuf[firbufPos + i + 10];
        sum2 = 0.0;
        for (j = 0; j < (((PsyModel.NSFIRLEN - 1) / 2) | 0) - 1; j += 2) {
          sum1 +=
            PsyModel.fircoef[j] *
            (firbuf[firbufPos + i + j] +
              firbuf[firbufPos + i + PsyModel.NSFIRLEN - j]);
          sum2 +=
            PsyModel.fircoef[j + 1] *
    ...
    for (chn = 0; chn < numchn; chn++) {
      let wsamp_l: Float32Array[];
      let wsamp_s: Float32Array[][];
      let en_subshort = new_float(12);
      let en_short: Float32Array = new Float32Array([0, 0, 0, 0]);
      let attack_intensity = new_float(12);
      let ns_uselongblock = 1;
      let attackThreshold: number;
      let max = new_float(CBANDS);
      let avg = new_float(CBANDS);
      let ns_attacks = new Int32Array([0, 0, 0, 0]);
      let fftenergy = new_float(HBLKSIZE);
      let fftenergy_s: Float32Array[] = new_float_n(3, HBLKSIZE_s) as Float32Array[];
      ...
      {
        let pf: Float32Array = ns_hpfsmpl[chn & 1];
        let pfPos = 0;
        for (i = 0; i < 9; i++) {
          let pfe = pfPos + (576 / 9) | 0;
          let p = 1.0;
          for (; pfPos < pfe; pfPos++)
      ...
      for (sblock = 0; sblock < 3; sblock++) {
        ...
        for (sb = 0; sb < SBMAX_s; sb++) {
          ...
          if (ns_attacks[sblock] >= 2 || ns_attacks[sblock + 1] === 1) {
            let idx = sblock !== 0 ? sblock - 1 : 2;
            let p = this.NS_INTERP(
      ...
      k = 0;
      {
        for (b = 0; b < gfc.npart_l; b++) {
          ...
          let eb2 = eb_l[kk] * PsyModel.tab[mask_idx_l[kk]];
          let ecb = gfc.s3_ll![k++] * eb2;
          while (++kk <= gfc.s3ind[b][1]) {
            eb2 = eb_l[kk] * PsyModel.tab[mask_idx_l[kk]];
          ...
          if (gfc.blocktype_old[chn & 1] === SHORT_TYPE)
            ...
            thr[b] = this.NS_INTERP(
              Math.min(
                ecb,
                Math.min(
                  PsyModel.rpelev * gfc.nb_1[chn][b],
                  PsyModel.rpelev2 * gfc.nb_2[chn][b],
                ),
              ),
              ecb,
              pcfact,
     ...
     *********************************************************************/
    for (chn = 0; chn < numchn; chn++) {
      let ppe: Float32Array;
      let ppePos = 0;
      let type: number;
      let mr: III_psy_ratio;
  ...
  private vbrpsy_compute_fft_l(
    ...
    if (chn < 2) {
      ...
      /* FFT data for mid and side channel is derived from L & R */
      for (let j = BLKSIZE - 1; j >= 0; --j) {
        let l = wsamp_l[wsamp_lPos + 0][j];
        let r = wsamp_l[wsamp_lPos + 1][j];
        wsamp_l[wsamp_lPos + 0][j] = (l + r) * Math.SQRT2 * 0.5;
        wsamp_l[wsamp_lPos + 1][j] = (l - r) * Math.SQRT2 * 0.5;
      }
    }
    ...
    for (let j = BLKSIZE / 2 - 1; j >= 0; --j) {
      let re = wsamp_l[wsamp_lPos + 0][BLKSIZE / 2 - j];
      let im = wsamp_l[wsamp_lPos + 0][BLKSIZE / 2 + j];
      fftenergy[BLKSIZE / 2 - j] = NON_LINEAR_SCALE_ENERGY(
        (re * re + im * im) * 0.5,
      );
    }
  ...
  private vbrpsy_compute_fft_s(
    ...
    if (chn === 2) {
      /* FFT data for mid and side channel is derived from L & R */
      for (let j = BLKSIZE_s - 1; j >= 0; --j) {
        let l = wsamp_s[wsamp_sPos + 0][sblock][j];
        let r = wsamp_s[wsamp_sPos + 1][sblock][j];
    ...
    for (let j = BLKSIZE_s / 2 - 1; j >= 0; --j) {
      let re = wsamp_s[wsamp_sPos + 0][sblock][BLKSIZE_s / 2 - j];
      let im = wsamp_s[wsamp_sPos + 0][sblock][BLKSIZE_s / 2 + j];
      fftenergy_s[sblock][BLKSIZE_s / 2 - j] = NON_LINEAR_SCALE_ENERGY(
        (re * re + im * im) * 0.5,
      );
    }
  }
  ...
  private vbrpsy_attack_detection(
    ...
    ns_attacks: Int32Array[],
    uselongblock: Int32Array,
  ): void {
    let ns_hpfsmpl: Float32Array[] = new_float_n(2, 576) as Float32Array[];
    const gfc = gfp.internal_flags!;
    let n_chn_out = gfc.channels_out;
    /* chn=2 and 3 = Mid and Side channels */
    let n_chn_psy =
      gfp.getMode() === MPEGMode.JOINT_STEREO ? 4 : n_chn_out;
    /* Don't copy the input buffer into a temporary buffer */
    /* unroll the loop 2 times */
    for (let chn = 0; chn < n_chn_out; chn++) {
      ...
      const firbuf = buffer[chn];
      let firbufPos = bufPos + 576 - 350 - PsyModel.NSFIRLEN + 192;
      assert(PsyModel.fircoef_.length === (((PsyModel.NSFIRLEN - 1) / 2) | 0));
      for (let i = 0; i < 576; i++) {
        let sum1: number, sum2: number;
        sum1 = firbuf[firbufPos + i + 10];
        sum2 = 0.0;
        for (let j = 0; j < (((PsyModel.NSFIRLEN - 1) / 2) | 0) - 1; j += 2) {
          sum1 +=
            PsyModel.fircoef_[j] *
            (firbuf[firbufPos + i + j] +
              firbuf[firbufPos + i + PsyModel.NSFIRLEN - j]);
          sum2 +=
            PsyModel.fircoef_[j + 1] *
            (firbuf[firbufPos + i + j + 1] +
              firbuf[firbufPos + i + PsyModel.NSFIRLEN - j - 1]);
        }
        ns_hpfsmpl[chn][i] = sum1 + sum2;
    ...
    for (let chn = 0; chn < n_chn_psy; chn++) {
      let attack_intensity = new_float(12);
      let en_subshort = new_float(12);
      let en_short = new Float32Array([0, 0, 0, 0]);
      let pf: Float32Array = ns_hpfsmpl[chn & 1];
      let pfPos = 0;
      ...
      for (let i = 0; i < 12; i++) {
        if (
          ns_attacks[chn][(i / 3) | 0] === 0 &&
          attack_intensity[i] > attackThreshold
        ) {
          ns_attacks[chn][(i / 3) | 0] = (i % 3) + 1;
        }
      }
  ...
  private vbrpsy_compute_masking_s(
    ...
    for (j = b = 0; b < gfc.npart_s; b++) {
      let kk = gfc.s3ind_s[b][0];
      let last = gfc.s3ind_s[b][1];
      let dd: number, dd_n: number;
      let x: number, ecb: number, avg_mask: number;
      dd = mask_idx_s[kk];
      dd_n = 1;
      ecb = gfc.s3_ss![j] * eb[kk] * PsyModel.tab[mask_idx_s[kk]];
      ++j;
      ++kk;
      while (kk <= last) {
        dd += mask_idx_s[kk];
        dd_n += 1;
        x = gfc.s3_ss![j] * eb[kk] * PsyModel.tab[mask_idx_s[kk]];
        ecb = this.vbrpsy_mask_add(ecb, x, kk - b);
        ++j;
        ++kk;
      }
      dd = ((1 + 2 * dd) / (2 * dd_n)) | 0;
      avg_mask = PsyModel.tab[dd] * 0.5;
      ecb *= avg_mask;
      thr[b] = ecb;
      gfc.nb_s2[chn][b] = gfc.nb_s1[chn][b];
      gfc.nb_s1[chn][b] = ecb;
  ...
  private vbrpsy_compute_masking_l(
    ...
    let k = 0;
    for (b = 0; b < gfc.npart_l; b++) {
      let x: number, ecb: number, avg_mask: number, t: number;
      /* convolve the partitioned energy with the spreading function */
      let kk = gfc.s3ind[b][0];
      let last = gfc.s3ind[b][1];
      let dd = 0, dd_n = 0;
      dd = mask_idx_l[kk];
      dd_n += 1;
      ecb = gfc.s3_ll![k] * eb_l[kk] * PsyModel.tab[mask_idx_l[kk]];
      ++k;
      ++kk;
      while (kk <= last) {
        dd += mask_idx_l[kk];
        dd_n += 1;
        x = gfc.s3_ll![k] * eb_l[kk] * PsyModel.tab[mask_idx_l[kk]];
      ...
      if (gfc.blocktype_old[chn & 0x01] === SHORT_TYPE) {
        let ecb_limit = PsyModel.rpelev * gfc.nb_1[chn][b];
        if (ecb_limit > 0) {
        ...
        let ecb_limit_2 = PsyModel.rpelev2 * gfc.nb_2[chn][b];
        let ecb_limit_1 = PsyModel.rpelev * gfc.nb_1[chn][b];
        let ecb_limit: number;
        if (ecb_limit_2 <= 0) {
          ecb_limit_2 = ecb;
        }
        if (ecb_limit_1 <= 0) {
  ...
  private vbrpsy_compute_MS_thresholds(
    ...
    let msfix2 = msfix * 2;
    let athlower = msfix > 0 ? Math.pow(10, athadjust) : 1;
    let rside: number, rmid: number;
    for (let b = 0; b < n; ++b) {
      let ebM = eb[2][b];
      let ebS = eb[3][b];
      let thmL = thr[0][b];
      ...
      if (thmL <= 1.58 * thmR && thmR <= 1.58 * thmL) {
        let mld_m = cb_mld[b] * ebS;
        let mld_s = cb_mld[b] * ebM;
      ...
      if (msfix > 0) {
        ...
        /***************************************************************/
        /* Naoki Shibata 2000 */
        let thmLR: number, thmMS: number;
        let ath = ath_cb[b] * athlower;
        thmLR = Math.min(Math.max(thmL, ath), Math.max(thmR, ath));
        thmM = Math.max(rmid, ath);
        thmS = Math.max(rside, ath);
        thmMS = thmM + thmS;
        if (thmMS > 0 && thmLR * msfix2 < thmMS) {
          let f = (thmLR * msfix2) / thmMS;
          thmM *= f;
          thmS *= f;
          assert(thmMS > 0);
        }
  ...
  L3psycho_anal_vbr(
    ...
    let eb: Float32Array[] = new_float_n(4, CBANDS) as Float32Array[];
    let thr: Float32Array[] = new_float_n(4, CBANDS) as Float32Array[];

    let sub_short_factor: Float32Array[] = new_float_n(4, 3) as Float32Array[];
    let pcfact = 0.6;

    /* block type */
    let ns_attacks: Int32Array[] = [
    ...
    /* chn=2 and 3 = Mid and Side channels */
    let n_chn_psy =
      gfp.getMode() === MPEGMode.JOINT_STEREO ? 4 : gfc.channels_out;
    ...
    /* LONG BLOCK CASE */
    {
      for (let chn = 0; chn < n_chn_psy; chn++) {
        ...
        this.vbrpsy_compute_loudness_approximation_l(
          gfp,
          gr_out,
          chn,
          fftenergy,
  ...
  // ====================================================================
  //  Spreading functions and helpers
  // ====================================================================

  private s3_func_x(bark: number, hf_slope: number): number {
    let tempx = bark, tempy: number;
  ...
  private norm_s3_func_x(hf_slope: number): number {
    let lim_a = 0, lim_b = 0;
    {
      let x = 0, l: number, h: number;
    ...
    }
    {
      ...
      let i: number;
      for (i = 0; i <= m; ++i) {
        let x = lim_a + (i * (lim_b - lim_a)) / m;
        let y = this.s3_func_x(x, hf_slope);
        sum += y;
      }
      {
        let norm = (m + 1) / (sum * (lim_b - lim_a));
        return norm;
      }
    }
  }
  ...
  private s3_func(bark: number): number {
    let tempx: number, x: number, tempy: number, temp: number;
    tempx = bark;
    if (tempx >= 0) tempx *= 3;
    else tempx *= 1.5;
  ...
  private init_numline(
    ...
    let b_frq = new_float(CBANDS + 1);
    let sample_freq_frac = sfreq / (sbmax > 15 ? 2 * 576 : 2 * 192);
    let partition = new_int(HBLKSIZE);
    let i: number;
    sfreq /= blksize;
    let j = 0;
    let ni = 0;
    ...
    for (let sfb = 0; sfb < sbmax; sfb++) {
      let i1: number, i2: number, start: number, end: number;
      let arg: number;
      start = scalepos[sfb];
      end = scalepos[sfb + 1];

      i1 = Math.floor(0.5 + deltafreq * (start - 0.5)) | 0;
      if (i1 < 0) i1 = 0;
      i2 = Math.floor(0.5 + deltafreq * (end - 0.5)) | 0;

      if (i2 > ((blksize / 2) | 0)) i2 = (blksize / 2) | 0;

      bm[sfb] = ((partition[i1] + partition[i2]) / 2) | 0;
      bo[sfb] = partition[i2];

      let f_tmp = sample_freq_frac * end;
      /*
  ...
  private init_s3_values(
    ...
    let s3: Float32Array[] = new_float_n(CBANDS, CBANDS) as Float32Array[];
    /*
    ...
    if (use_old_s3) {
      for (let i = 0; i < npart; i++) {
        for (j = 0; j < npart; j++) {
          let v = this.s3_func(bval[i] - bval[j]) * bval_width[j];
          s3[i][j] = v * norm[i];
        }
      }
    } else {
      for (j = 0; j < npart; j++) {
        let hf_slope = 15 + Math.min(21 / bval[j], 12);
        let s3_x_norm = this.norm_s3_func_x(hf_slope);
        for (let i = 0; i < npart; i++) {
          let v =
            s3_x_norm *
    ...
    let p = new_float(numberOfNoneZero);
  ...
  private stereo_demask(f: number): number {
    /* setup stereo demasking thresholds */
    /* formula reverse enginerred from plot in paper */
  ...
  psymodel_init(gfp: LameGlobalFlags): number {
    ...
    let i: number;
    let useOldS3 = true;
    let bvl_a = 13, bvl_b = 24;
    let snr_l_a = 0, snr_l_b = 0;
    let snr_s_a = -8.25, snr_s_b = -4.5;

    let bval = new_float(CBANDS);
    let bval_width = new_float(CBANDS);
    let norm = new_float(CBANDS);
    const sfreq = gfp.getOutSampleRate();
    ...
    gfc.blocktype_old[0] = gfc.blocktype_old[1] = NORM_TYPE;
    ...
    for (i = 0; i < 4; ++i) {
      ...
      for (let j = 0; j < 9; j++)
        gfc.nsPsy.last_en_subshort[i][j] = 10.0;
    }
    ...
    for (i = 0; i < gfc.npart_l; i++) {
      ...
      x -= 8.0;
      gfc.minval_l[i] = Math.pow(10.0, x / 10.0) * gfc.numlines_l[i];
    }
     ...
     * per second
     */
    let frame_duration = (576.0 * gfc.mode_gr) / sfreq;
    gfc.ATH!.decay = Math.pow(10.0, (-12.0 / 10.0) * frame_duration);
    ...
    if (gfp.ATHtype !== -1) {
      ...
      let freq: number;
      const freq_inc = gfp.getOutSampleRate() / BLKSIZE;
      let eql_balance = 0.0;
      freq = 0.0;
      for (i = 0; i < BLKSIZE / 2; ++i) {
        ...
        freq += freq_inc;
        gfc.ATH!.eql_w[i] =
          1.0 / Math.pow(10, this.ATHformula(freq, gfp) / 10);
    ...
    for (i = 0; i < gfc.npart_l; i++) {
      const freq =
        (sfreq * (j + gfc.numlines_l[i] / 2)) / (1.0 * BLKSIZE);
    ...
    for (i = 0; i < gfc.npart_s; i++) {
      const freq =
        (sfreq * (j + gfc.numlines_s[i] / 2)) / (1.0 * BLKSIZE_s);
